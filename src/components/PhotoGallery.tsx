...
export default function PhotoGallery() {
    ...
    useEffect(() => {
        if (photos.length) runPrefetch(selectedIndex ?? 0);
    }, [photos, selectedIndex, runPrefetch]);
    ...
    const openLightbox = useCallback((index: number) => {
        resetView();
        setAnimate(false);          // no slide on open
        setSlotK(1);
    ...
    const onTrackTransitionEnd = useCallback((e: React.TransitionEvent) => {
        ...
        if (slotK === 2) setSelectedIndex(i => (i === null ? i : (i + 1) % n));
        else if (slotK === 0) setSelectedIndex(i => (i === null ? i : (i - 1 + n) % n));
        setSlotK(1);
    ...
    }, [selectedIndex, closeLightbox, handlePrevious, handleNext]);

    const n = photos.length;
    // The 3-slot window around the current photo.
    const slots = selectedIndex === null
        ? []
        : [(selectedIndex - 1 + n) % n, selectedIndex, (selectedIndex + 1) % n];
    ...
    return (
        <div>
            ...
            {/* Lightbox — portaled to <body>, pinned to the viewport */}
