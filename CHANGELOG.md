...
### Added
- **Bulk editing on the guest list** — with guests ticked, the action bar gained one-click **⚠️ Issue** and **📌 Need** flag buttons (idempotent toggles: if every selected guest already has the flag, the button clears it) and an **✏️ Edit Selected…** modal covering flag, note, side and RSVP status. Every field defaults to *Leave unchanged* so a bulk edit only writes what was actually set. Notes have three modes — **Add to existing** (default; appends on a new line via `CASE WHEN COALESCE(NULLIF(TRIM(notes),''),'') = '' THEN … ELSE notes || E'\n' || … END`, so existing notes are never clobbered and guests with no note don't get a leading blank line), **Replace**, and **Clear**. A green confirmation pill reports what changed. Backed by a widened `PATCH /api/admin/guest-list` that now accepts `{ ids: [...], flag?, side?, invited?, rsvp_status?, notes?, noteMode? }` and builds the `SET` clause from only the keys present, updating in a single statement via `WHERE id = ANY($n)`; the original `{ id, address }` single-guest shape is untouched so the CSV address-reconcile tool keeps working.
